...
import { Loader2, Check, X, Video, ExternalLink, Clock } from 'lucide-react'
...
export default function MentorSessions() {
  ...
  const getStatusBadge = (status: Session['status']) => {
    switch (status) {
      ...
      case 'completed':
        return <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100 border-none">Completed</Badge>
      default:
  ...
  return (
    <DashboardLayout userRole="MENTOR">
      <div className="p-8 space-y-8 max-w-7xl mx-auto">
        {/* Page Header */}
        <div>
          <h1 className="text-4xl font-black text-[#4A3F33] tracking-tight mb-2">My Schedule</h1>
          <p className="text-[#9B8B7E] font-medium">Manage student bookings and approve mentorship requests.</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 space-y-6">
            <Card className="bg-white border-[#E8DFD3] shadow-sm rounded-3xl overflow-hidden">
              <CardHeader className="bg-[#F8F3EE] border-b border-[#E8DFD3]">
                <CardTitle className="text-[#4A3F33] text-lg font-bold">Select Date</CardTitle>
              </CardHeader>
            ...
            <div className="bg-[#6B5844] rounded-3xl p-6 text-white shadow-lg">
          ...
          <div className="lg:col-span-8">
            <Card className="bg-white border-[#E8DFD3] shadow-sm rounded-3xl overflow-hidden min-h-[500px]">
              <CardHeader className="flex flex-row items-center justify-between border-b border-[#E8DFD3] bg-[#F8F3EE]/30">
                <CardTitle className="text-[#4A3F33] text-lg font-bold">
                  Sessions for {selectedDate ? format(selectedDate, 'MMMM d, yyyy') : 'All Dates'}
                </CardTitle>
                <Badge variant="outline" className="border-[#6B5844] text-[#6B5844] font-bold">
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-24 text-[#9B8B7E]">
                    <div className="w-20 h-20 bg-[#F5F0EA] rounded-full flex items-center justify-center mb-4">
                      <Calendar className="h-8 w-8 text-[#D4C4B0]" />
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    <Table>
                      <TableHeader>
                        <TableRow className="border-b border-[#E8DFD3] hover:bg-transparent">
                          <TableHead className="text-[#9B8B7E] uppercase font-black text-[10px] tracking-widest pl-6">Student</TableHead>
                          <TableHead className="text-[#9B8B7E] uppercase font-black text-[10px] tracking-widest">Time & Topic</TableHead>
                          <TableHead className="text-[#9B8B7E] uppercase font-black text-[10px] tracking-widest text-center">Status</TableHead>
                          <TableHead className="text-[#9B8B7E] uppercase font-black text-[10px] tracking-widest text-right pr-6">Actions</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {filteredSessions.map((session) => (
                          <TableRow key={session.id} className="border-b border-[#F5F0EA] last:border-0 hover:bg-[#FDFBF9] transition-colors">
                            <TableCell className="pl-6 py-6">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-[#E8DFD3] flex items-center justify-center text-[#6B5844] font-bold text-sm">
                                  {session.studentName[0]}
                                </div>
                                <span className="font-bold text-[#4A3F33]">{session.studentName}</span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div className="space-y-1">
                                <div className="flex items-center gap-2 text-[#4A3F33] font-bold text-sm">
                                  <Clock size={14} className="text-[#6B5844]" />
                                  {format(session.time, 'hh:mm a')}
                                </div>
                                <div className="text-xs text-[#9B8B7E] font-medium uppercase tracking-tight line-clamp-1 max-w-[150px]">
                                  {session.topic}
                                </div>
                              </div>
                            </TableCell>
                            ...
                            <TableCell className="pr-6 text-right">
                              <div className="flex items-center justify-end gap-2">
                                {session.status === 'pending' ? (
                                  ...
                                  <Button
                                    size="sm"
                                    className="bg-[#6B5844] hover:bg-[#4A3F33] text-white h-9 px-4 rounded-xl shadow-md transition-all font-bold flex items-center gap-2"
                                  ...
                                  <span className="text-xs text-[#D4C4B0] font-bold uppercase italic">No actions</span>
                                )}
                              </div>
                            </TableCell>
                          </TableRow>
